...
author: Emily Bauser and Prof Fisk
---
...
FILENAME<-("/Users/emilybauser/Desktop/breastFNA.csv")
...
vector1<-c(1:569)
new_vector<-sample(vector1, 100, replace=FALSE, prob=NULL)
...
factors are a type of  data type that allow for ordered categories that only accepts fixed values
...
16 had an accuracy value of 0.9616768 which is the highest out of the three mtry values
...
y<-print(testDat$Prediction!=testDat$Diagnosis)
x<-sum(y, na.rm=TRUE)
```
...
It appears that benign tumors were more often classified as malignant than malignant were classified as benign. 4 misclassifications overall 
...
It would overall be better a better health outcome is a benign tumor got classfied as malignant because most likely the patient would just get it removed and thats better than leaving a malignant tumor alone. Perhaps knowing where the tumor is located along with the size would help inform decisions about next steps as well. 
...
It should be about 50% accuracy if you only have two options.
...
set.seed(42666) #just makes it easier for Dr. Fisk to grade
#make two vectors to test and train
vector1<-c(1:569)
new_vector<-sample(vector1, 100, replace=FALSE, prob=NULL)
testDat<-all_breast_data[new_vector,]
trainDat<-all_breast_data[-new_vector,]
...
#now we are going to train the model
#we are using the trying earth algorithm so put degree, which is 'rf' and what is called a 5 fold cross-validation where the algorithm is trained on random subsets of the data and tested to get initial accuracy. 
model<-train(Diagnosis~.,
            data=trainDat,
            method ='degree',
            trControl=trainControl(method = "cv",number = 5)
...
y<-print(testDat$Prediction!=testDat$Diagnosis)
x<-sum(y, na.rm=TRUE)
#give me real summary of errors
print(summary(testDat[which(testDat$Diagnosis!=testDat$Prediction),"Diagnosis"]))
#print table of accuracy results
print(table(all_breast_data$Diagnosis))
print(table(all_breast_data$Diagnosis)/nrow(all_breast_data))
#preprosessing is now happening
model2<-train(Diagnosis~.,
            data=trainDat,
            method ='degree',
            preProc=c("center","scale"), #note this line
...
I really enjoyed the first half of this assignment and felt very accomplished that I could figure out a lot of what needed to get done to make the model work. I struggled a bit more in the second half using a new model mostly because a section of the code provided in the first half didn't seem to work in either case but it became more of a roadblock here. My code is missing object 'train' but I am not sure where it would have been created or why it isn't being named. Overall I enjoyed this assignment and feel comftorable working with machine learning to do this kind of work and training models. 
