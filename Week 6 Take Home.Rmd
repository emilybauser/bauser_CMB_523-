...
```{r}
n= 100000#pick a large number for N, around one-hundred million or so
res = runif(n) + 1
```

QUESTION 1: What does the above code do?

ANSWER 1: <First the code defines n. The code then creates a function, and calls it res, that is used in later chunks of code. The runif function creates a string of numbers greater than 1 that is 100,000,000 numbers long. No min or max is set so it will just create a string of 100000000 elements>
...
ANSWER 2: the i is being used as a sort of placeholder. if i was replaced with a different number such as two then only the second value in the vector so it is acessing the "ith" value in the vector which keeps it unlimited. 
...
```{r}
#Method 1
sequencing_matrix <- as.data.frame(matrix(sample(x = c("a","C","T","G"),size = 22000,replace = T), nrow=1000))
...
i and j are open variables in the code. We are looking at the entire matrix not just a specific row so but using variables like i and j we are basically telling the computer to look to the i-th or j-th much like we do with nth in other situations. They are the row and column that we want the code to pull from but because we want to look at everything in the matrices we use letters. 
...
<it implies that nrow and ncol were being called every time the loop ran through which is part of what makes the first code run slower than this newer code.>
...
<this code should run 1000 times, once for each nrow>
...
```{r}
#Your code here
#reprint the sequencing data just to have it down here as well
sequencing_matrix <- as.data.frame(matrix(sample(x = c("a","C","T","G"),size = 22000,replace = T), nrow=1000))
colnames(sequencing_matrix)<-paste0("Loci",1:ncol(sequencing_matrix))
rownames(sequencing_matrix)<-paste0("Sample",1:nrow(sequencing_matrix))
